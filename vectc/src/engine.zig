...
// vectc engine — shared by the native CLI and the web (wasm) build.
// Platform differences go through hooks / use_memfs; engine code itself
// never touches std.process, std.debug, or OS files directly, so the exact
// same source compiles to a native exe and to wasm32-freestanding.

// Host hooks. The native driver wires these to stdio/exit/OS files; the web
// driver wires them to buffers/trap/memory files. Always set before running.
// The engine itself never touches std.process, std.debug, or OS files, so the
// exact same source compiles to a native exe and to wasm32-freestanding.
pub var out_hook: ?*const fn ([]const u8) void = null;
pub var fatal_hook: ?*const fn ([]const u8) void = null;
pub var clock_hook: ?*const fn () u64 = null;
pub var stdin_hook: ?*const fn ([]u8) usize = null; // fill buf, return bytes (0 = EOF)
pub var fs_hook: ?*const FsOps = null;

pub const FsErr = error{ NotFound, Denied, IoFail, TooBig };

pub const FsOps = struct {
    open_read: *const fn (alloc: std.mem.Allocator, path: []const u8) FsErr!usize,
    open_write: *const fn (alloc: std.mem.Allocator, path: []const u8) FsErr!usize,
    read: *const fn (id: usize, buf: []u8) FsErr!usize, // 0 = EOF
    write: *const fn (id: usize, bytes: []const u8) FsErr!void,
    close: *const fn (id: usize) void,
    read_all: *const fn (alloc: std.mem.Allocator, path: []const u8, limit: usize) FsErr![]u8,
};

fn fs() *const FsOps {
    return fs_hook orelse unreachable;
}
pub fn fail(lineno: usize, comptime fmt: []const u8, args: anytype) noreturn {
    // NOTE: wasm32-ReleaseSmall codegen is fragile here. Verified rules:
    // value-fallback catches only (never `catch unreachable`), sequential
    // appends (never tuple-concat with globals), exactly ONE hook call,
    // address-taken callees must return normally. Do NOT "simplify".
    ...
    n += (std.fmt.bufPrint(fb[n..], " at line ", .{}) catch fb0).len;
    n += (std.fmt.bufPrint(fb[n..], "{s}", .{intStr(&nb, @as(i64, @intCast(lineno)))}) catch fb0).len;
    if (fail_file.len > 0) {
        n += (std.fmt.bufPrint(fb[n..], " (", .{}) catch fb0).len;
        const fl = fail_file;
        n += (std.fmt.bufPrint(fb[n..], "{s}", .{fl}) catch fb0).len;
        n += (std.fmt.bufPrint(fb[n..], ")", .{}) catch fb0).len;
    }
    n += (std.fmt.bufPrint(fb[n..], "\n[Vect Execution Terminated]\n", .{}) catch fb0).len;
    if (fatal_hook) |h| h(fb[0..n]);
    unreachable;
}

pub var fail_file: []const u8 = "";
...
pub const Compiler = struct {
    ...
    fn compileStmt(self: *Compiler, idx: usize) error{OutOfMemory}!usize {
        ...
        // ---- lo (guarded: only lo<digits> is a loop) ----
        ...
        // ---- in (guarded: ind0-style names must not match) ----
        if (std.mem.eql(u8, t, "in") or std.mem.startsWith(u8, t, "in ") or std.mem.startsWith(u8, t, "in\t") or std.mem.startsWith(u8, t, "in(")) {
            var name: []const u8 = "";
        ...
        // ---- vga (guarded: vga<digits> presets only) ----
        if (std.mem.startsWith(u8, t, "vga") and t.len > 3 and t[3] >= '0' and t[3] <= '9') {
    ...
    pub fn compileProgram(alloc: std.mem.Allocator, lines: []Line, prog: *Program) error{OutOfMemory}!void {
...
pub const Vm = struct {
    ...
    fn out(self: *Vm, bytes: []const u8) void {
        _ = self;
        if (out_hook) |h| h(bytes) else unreachable;
    }
    ...
    fn readLine(self: *Vm, buf: []u8) []u8 {
        _ = self;
        const h = stdin_hook orelse unreachable;
        var n: usize = 0;
        while (n < buf.len) {
    ...
    fn kvParse(self: *Vm, kv: *KV, data: []const u8, ln: u32) error{OutOfMemory}!void {
        var p: usize = 0;
        while (p < data.len) {
    ...
    fn kvLoad(self: *Vm, kv: *KV, ln: u32) error{OutOfMemory}!void {
        const data = fs().read_all(self.alloc, kv.path, 8 * 1024 * 1024) catch |e| {
            if (e == FsErr.NotFound) return; // first run: start empty
            fail(ln, "Cannot open store '{s}'", .{kv.path});
        };
        try self.kvParse(kv, data, ln);
    }

    fn kvRender(self: *Vm, kv: *KV, ln: u32) error{OutOfMemory}![]u8 {
        ...
        for (kv.keys.items, 0..) |k, i| {
            ...
            try rb.appendSlice(self.alloc, rec);
        }
        return rb.toOwnedSlice(self.alloc);
    }

    fn kvSave(self: *Vm, kv: *KV, ln: u32) error{OutOfMemory}!void {
        const data = try self.kvRender(kv, ln);
        const id = fs().open_write(self.alloc, kv.path) catch fail(ln, "Cannot write store '{s}'", .{kv.path});
        fs().write(id, data) catch fail(ln, "Cannot write store '{s}'", .{kv.path});
        fs().close(id);
    }

    pub     fn run(self: *Vm, entry: usize) error{OutOfMemory}!void {
        if (clock_hook == null) self.t0 = std.Io.Clock.Timestamp.now(self.io, .boot);
        self.frames = try self.alloc.alloc(Frame, 64);
        self.depth = 0;
        ...
        while (self.depth > 0) {
            ...
            switch (ins.op) {
                ...
                .clock => {
                    if (clock_hook) |h| {
                        self.push(VVal{ .tag = .Int, .int = @intCast(h()) });
                        continue;
                    }
                ...
                .fopen => {
                    ...
                    if (std.mem.eql(u8, modev.str, "r")) {
                        id = fs().open_read(self.alloc, pathv.str) catch |e| {
                            if (e == FsErr.NotFound) fail(ln, "Cannot open '{s}'", .{pathv.str});
                            fail(ln, "Cannot open '{s}'", .{pathv.str});
                        };
                    } else if (std.mem.eql(u8, modev.str, "w")) {
                        id = fs().open_write(self.alloc, pathv.str) catch fail(ln, "Cannot create '{s}'", .{pathv.str});
                ...
                .fread => {
                    ...
                    var got: usize = 0;
                    while (got < buf.len) {
                        const r = fs().read(@intCast(fhv.int), buf[got..]) catch fail(ln, "File read failed", .{});
                        if (r == 0) break;
                        got += r;
                    }
                ...
                .fwrite => {
                    ...
                    switch (v.tag) {
                        .Str => fs().write(id, v.str) catch fail(ln, "File write failed", .{}),
                        .Int => {
                            var nb: [32]u8 = undefined;
                            fs().write(id, intStr(&nb, v.int)) catch fail(ln, "File write failed", .{});
                        },
                        .Float => {
                            const s = std.fmt.allocPrint(self.alloc, "{d}", .{v.float}) catch fail(ln, "File write failed", .{});
                            fs().write(id, s) catch fail(ln, "File write failed", .{});
                        },
...
pub fn dumpProg(prog: *Program) void {
...
pub fn joinPath(alloc: std.mem.Allocator, dir: []const u8, name: []const u8) error{OutOfMemory}![]const u8 {
...
pub fn fileDir(path: []const u8) []const u8 {
    if (std.mem.lastIndexOfAny(u8, path, "/\\")) |sp| return path[0 .. sp + 1];
    return "";
}

pub fn parseInto(list: *std.ArrayList(Line), alloc: std.mem.Allocator, content: []const u8, fname: []const u8, dir: []const u8, is_header: bool) error{OutOfMemory}!void {
...
pub fn runCli(init: std.process.Init) !void {
    ...
    const lines = try buildLines(alloc, init.io, path);
    ...
    var vm = Vm{ .alloc = alloc, .io = init.io, .prog = &prog, .arrays = .empty, .files = .empty, .wopens = .empty, .kvs = .empty };
    try vm.run(top);
}
fn nativeOut(bytes: []const u8) void {
    std.Io.File.stdout().writeStreamingAll(native_io, bytes) catch {};
}

var native_io: std.Io = undefined;

fn nativeFatal(msg: []const u8) noreturn {
    std.debug.print("{s}", .{msg});
    std.process.exit(1);
}

pub fn buildLines(alloc: std.mem.Allocator, io: std.Io, path: []const u8) ![]Line {
    const content = std.Io.Dir.cwd().readFileAlloc(io, path, alloc, .limited(10 * 1024 * 1024)) catch {
    ...
    while (scan < lines_list.items.len) : (scan += 1) {
        ...
        const ic = std.Io.Dir.cwd().readFileAlloc(io, full, alloc, .limited(2 * 1024 * 1024)) catch {
            fail(ln.lineno, "Cannot open header '{s}'", .{full});
        };
        try parseInto(&lines_list, alloc, ic, full, fileDir(full), true);
    }
    return lines_list.toOwnedSlice(alloc);
}
